...
public class EnemyBrain : MonoBehaviour
{
    ...
    public IHealer Healer;          // optional
    ...
    void Awake()
    {
        ...
        if (hp != null) hp.OnDied += _ => HandleDeath();
    }
    ...
    void WireComponents()
    {
        ...
        // Resolve interfaces from concrete components on the same object
        ...
        // ✅ explicit null checks (no bool conversion on interfaces)
    ...
    void Update()
    {
        // Safety net: re-wire if something was missing at load time
        if (!wired) WireComponents();
        if (!wired)
        {
            if (!loggedMissing)
            {
                Debug.LogWarning($"{name}/EnemyBrain missing comps → " +
                                 $"Move:{(Move!=null)} Combat:{(Combat!=null)} Sense:{(Sense!=null)} Agent:{(agent!=null)} Health:{(hp!=null)}");
                loggedMissing = true;
            }
            return;
        }

        if (Combat.IsDead) { HandleDeath(); return; }
        ...
        // Build inputs and run the tiny NN
        ...
        {
            // Fallbacks in case chosen slot doesn’t exist
    ...
    {
        if (!enabled) return;
        var agen = GetComponent<UnityEngine.AI.NavMeshAgent>();
        if (agen != null) { agen.isStopped = true; agen.ResetPath(); }
        fitness -= penalty;
        enabled = false;
        OnEpisodeEnd?.Invoke(fitness);
    }
