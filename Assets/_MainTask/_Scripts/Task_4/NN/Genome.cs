...
/// Holds weights for the simple 1-hidden-layer NN and a fitness score.
[Serializable]
public class Genome
{
    ...
    /// Creates a new genome with a network of the given shape.
    ...
    /// Deep clone (copies all weights/biases; resets fitness).
    public Genome Clone()
    {
        ...
        for (int i = 0; i < I; i++)
            for (int h = 0; h < H; h++)
                if (UnityEngine.Random.value < rate)
                    net.w1[i, h] += NextGaussian() * sigma;

        for (int h = 0; h < H; h++)
            if (UnityEngine.Random.value < rate)
                net.hBias[h] += NextGaussian() * sigma;

        for (int h = 0; h < H; h++)
            for (int o = 0; o < O; o++)
                if (UnityEngine.Random.value < rate)
                    net.w2[h, o] += NextGaussian() * sigma;

        for (int o = 0; o < O; o++)
            if (UnityEngine.Random.value < rate)
                net.oBias[o] += NextGaussian() * sigma;
    }
}
